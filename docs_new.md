...
# Mechanistyczna Interpretowalność Modelu GPT w Środowisku Tic-Tac-Toe

<br><br>

**Autorzy:** Szymon Daniel Olkowski, Dominik Kwidziński, Wiktor Laska
...
### 4.3.1. Geometryczna Abstrakcja Taktyki (Analiza PCA)

W oryginalnym badaniu modelu Othello-GPT, wizualizacja przestrzeni ukazała idealną, płaską siatkę 2D odpowiadającą fizycznej planszy. W przypadku modelu Kółko i Krzyżyk, rzutowanie za pomocą algorytmu PCA ujawniło abstrakcyjny, zniekształcony kształt.

Nie jest to błąd modelu. W grze Kółko i Krzyżyk fizyczne położenie na płaskiej kartce nie ma znaczenia decyzyjnego. Liczy się tylko strategiczna funkcja konkretnego pola. Model zauważa to w trakcie treningu i całkowicie porzuca klasyczną, płaską geometrię.

Zamiast niej, wygina przestrzeń, grupując wektory pól według ich symetrii taktycznej. Trójwymiarowy rzut ukazuje wyraźny podział abstrakcyjny wewnątrz sieci:
* **Środek** (pole 4) zostaje odizolowany jako najważniejszy, samodzielny punkt.
* **Narożniki** (pola 0, 2, 6, 8) tworzą jeden wspólny klaster bardzo silnych pól taktycznych.
* **Krawędzie** (pola 1, 3, 5, 7) zostają wypchnięte do kolejnej grupy jako pola o najsłabszym potencjale wygrywającym.

To zjawisko stanowi ostateczny dowód na to, że sieć operuje zaawansowaną wiedzą o grze. Z uwagi na dużą symetrię środowiska, organizuje ją jednak w mocno nieliniową strukturę 3D, zamiast odtwarzać płaski obraz widziany okiem człowieka. Włączane wykresy 3D HTML potwierdzające ten rozkład zostały wygenerowane i dołączone jako materiał dodatkowy.
